...
;Conversion of 4-digit HEX to BCD and 5-digit BCD to HEX.
;---------------------------------------------------------------------
section .data
	nline		db	10,10
	nline_len:	equ	$-nline
...
section .text
	global _start
_start:
	print	msg,msg_len	; "MIL assignment 03 : Hex to BCD & BCD to Hex"
	print	menu, menu_len
...
hex_bcd:
	...
	call 	accept_16            ; This is the used to convert accepted numbers into into HEX formate.. Ok
	mov 	ax,bx                        bx is havind 4 digit HEX numbers for example:- 3456 H (16 bit)
				; Copy the hex number into ax register i.e. 16 bit 
	mov 	rbx,10            ; mov immediate number 10 to the rbx register.. 
back:
	xor 	rdx,rdx          ;  rdx =0. 
	div 	rbx 

	push 	dx               ; push msb 4 on stack
	inc 	byte[digitcount]
...
;------------------------------------------------------------------
accept_16:
	read	buf,5		; buflen = 4 + 1  ENTER                    ; INPUT 459F H and output 10322 BCD numbers
...
cnt:	mov 	rdx,0		; make rdx=0 (as in div instruction rdx:rax/rbx)
	...
	div 	rbx
	cmp 	dl, 09h		; check for remainder in RDX               ; print in reverse manner. 
	jbe  	add30
	add  	dl, 07h 
add30:
